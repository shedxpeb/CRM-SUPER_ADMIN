...
### crm-migration.sql
- **Target Database**: `BuildX` (CRM database)
- **Purpose**: Updates CRM database tables for Organization, User, Role, Permission, and related entities
- **Usage**: 
  ```bash
  psql -h 127.0.0.1 -p 5432 -U postgres -d BuildX -f crm-migration.sql
  ```
...
- **CRM DB (`BuildX`)**: Use manual SQL scripts in this directory
